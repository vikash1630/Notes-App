...
const Pinned = () => {
  ...
  // Update pinnedNotes whenever notes change
  useEffect(() => {
    const filtered = notes.filter(note => note.pin);
    setPinnedNotes(filtered);
  }, [notes]);
    ...
    <div className="bg-blue-950 h-[95vh] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-6  flex flex-wrap gap-6 justify-start">
        {pinnedNotes.length > 0 ? (
          ...
          <h1 className="text-white text-3xl mt-6 w-full text-center">No pinned notes</h1>
        )}
      </div>
    </div>
  );
